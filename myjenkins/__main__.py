...
@click.option('-c', '--class-names-only', is_flag=True)
def failures(o, job, build_id, class_names_only):
    """Print failed tests for a build and its children."""
    results = visitor.FailedTestCollector(o.client).visit(o.client[job][build_id])

    if class_names_only:
        results = set(r.className for r in results)
    else:
        results = set(r.identifier() for r in results)
